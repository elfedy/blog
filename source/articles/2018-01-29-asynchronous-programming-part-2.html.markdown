...
All that we have seen here is asynchronous code and the same rules apply for it. In particular, we are still running code in a single threaded way. When an event that has a listener associated to it occurs, the callback is added to the execution queue and run when its turn comes.
